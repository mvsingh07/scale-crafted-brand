...
// ── Generic mock for DB-sourced projects without a dedicated animation ────────
const GenericMock = ({ stack }: { stack: string[] }) => (
  <div className="space-y-3">
    <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Stack</div>
    <div className="flex flex-wrap gap-2">
      {stack.map((t) => (
        <span key={t} className="rounded-md border border-border/60 bg-muted/30 px-2 py-1 font-mono text-[10px] text-muted-foreground">
          {t}
        </span>
      ))}
    </div>
...
// Map known project titles → their animated mock components
const MOCK_MAP: Record<string, React.ComponentType> = {
  "Real-Time Auction Platform": AuctionMock,
  "File Processing Microservice": FileProcessingMock,
  "AI Health Engine": AIHealthMock,
};

// ── Default (hardcoded) project data ─────────────────────────────────────────
const DEFAULT_PROJECTS: Project[] = [
  { id: "1", profile_id: "", number: "01", title: "Real-Time Auction Platform", tagline: "Live bidding for 50K+ concurrent users.", problem: "Auctions where every millisecond matters — bids must be globally consistent, fair, and instantly reflected for every viewer.", solution: "Event-driven core on Kafka and RabbitMQ with Redis-backed bid state, WebSocket fan-out, and idempotent write paths to guarantee correctness under bursty load.", impact: "Supported 50K+ concurrent users while reducing end-to-end latency from 500ms to 120ms.", stack_tags: ["NestJS", "Kafka", "RabbitMQ", "Redis", "Next.js", "Supabase", "AWS S3"], accent: "from-[hsl(var(--brand-cyan))] to-[hsl(var(--brand-violet))]", ord: 0 },
  { id: "2", profile_id: "", number: "02", title: "File Processing Microservice", tagline: "Encrypted streaming for 1GB+ payloads.", problem: "Single-shot uploads of large files were failing, blocking memory, and leaking sensitive data on retries.", solution: "Chunked, resumable uploads with AES-256 and RSA encryption, BullMQ background jobs, parallel worker processing, RBAC, and signed URL delivery.", impact: "Reliable processing of 1GB+ encrypted files with constant memory footprint and secure access controls.", stack_tags: ["NestJS", "AES-256", "RSA", "BullMQ", "AWS S3", "RBAC", "Postgres"], accent: "from-[hsl(var(--brand-violet))] to-[hsl(var(--brand-pink))]", ord: 1 },
  { id: "3", profile_id: "", number: "03", title: "AI Health Engine", tagline: "Voice-first AI for clinical workflows.", problem: "Care teams needed instant, contextual answers from health data without typing or navigating dashboards.", solution: "RAG pipeline blending LLaMA and OpenAI with Supabase vector search and ElevenLabs voice interaction behind a hardened API.", impact: "Reduced query-to-insight time from minutes to seconds with conversational, multi-modal access.", stack_tags: ["Python", "LLaMA", "OpenAI", "ElevenLabs", "Supabase", "FastAPI", "Vector DB"], accent: "from-[hsl(var(--brand-pink))] to-[hsl(var(--brand-cyan))]", ord: 2 },
];

// ── Legacy hardcoded list kept for Mock component mapping ─────────────────────
const projects = [
  {
    n: "01",
...
export const Projects = ({ projects: dbProjects }: ProjectsProps = {}) => {
  const items = dbProjects && dbProjects.length > 0 ? dbProjects : DEFAULT_PROJECTS;
    ...
    <section id="work" className="relative py-24 md:py-32">
      <div className="container">
        ...
        <div className="mt-16 space-y-10">
          {items.map((p, i) => {
            const Mock = MOCK_MAP[p.title] ?? (() => <GenericMock stack={p.stack_tags as string[]} />);
            return (
              <motion.article
                key={p.id}
              ...
              >
                ...
                <div className="lg:col-span-5">
                  <motion.div
                    ...
                    className="animate-mock-float relative h-full overflow-hidden rounded-2xl border border-border/70 bg-[hsl(var(--surface-2))] p-1 transition-all duration-500 group-hover:shadow-[0_0_48px_-8px_hsl(var(--brand-cyan)/0.35)] group-hover:border-border"
                  >
