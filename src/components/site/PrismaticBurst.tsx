...
export function PrismaticBurst({
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      program.uniforms.uMouse.value = smooth as [number, number];
    ...
    return () => {
      ...
      try {
        ...
        // Canvas may already be gone during hot reload teardown.
      }
      ...
      try {
        if (gradTexRef.current?.texture) gl.deleteTexture(gradTexRef.current.texture);
      } catch {
  ...
  useEffect(() => {
    ...
    program.uniforms.uAnimType.value =
      { rotate: 0, rotate3d: 1, hover: 2 }[animationType ?? "rotate3d"];
