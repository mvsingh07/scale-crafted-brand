...
export function AboutSection() {
  ...
  const paragraphs: AboutParagraph[] = identity?.about_paragraphs?.length ? identity.about_paragraphs : DEFAULT_PARAGRAPHS;
  const stats: AboutStat[]           = identity?.about_stats?.length      ? identity.about_stats      : DEFAULT_STATS;
          ...
          {paragraphs.map((p, i) => (
            <motion.p
              ...
              style={{
                ...
                fontSize: "clamp(15px, 1.7vw, 18px)", lineHeight: 1.85, margin: 0,
                color: p.italic ? MUTED : SILVER,
          ...
          <div style={{
            ...
            {stats.map((s, i) => (
              <div key={i}>
                ...
                <p style={{ fontFamily: "var(--font-inter), Inter, sans-serif", fontSize: 12, letterSpacing: "0.1em", color: MUTED, marginTop: 4 }}>{s.label}</p>
                {i < stats.length - 1 && (
