...
const BLOG_FILTERS: { label: string; value: FilterValue }[] = [
  { label: "All",      value: "all"      },
    ...
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "64px 32px 80px" }}>
      ...
      {loading ? (
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {filtered.map(blog => <BlogRow key={blog.id} blog={blog} />)}
          </motion.div>
        </AnimatePresence>
      )}
    </div>
...
function BlogRow({ blog: b }: { blog: EcosystemBlog }) {
  ...
  return (
    ...
    >
      {b.image_url && (
        <div style={{ width: 88, height: 64, borderRadius: 8, overflow: "hidden", background: "rgba(255,255,255,0.04)", flexShrink: 0 }}>
          <img src={b.image_url} alt={b.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        </div>
      )}
      <div style={{ minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
          <span style={{ background: platform.bg, borderRadius: 5, padding: "2px 8px", fontFamily: "var(--font-inter), Inter, sans-serif", fontSize: 9, letterSpacing: "0.1em", color: platform.color }}>
            {platform.label}
          </span>
          {b.published_at && (
            <span style={{ fontFamily: "var(--font-inter), Inter, sans-serif", fontSize: 10, color: MUTED }}>
              {new Date(b.published_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
            </span>
          )}
        </div>
        <h3 style={{ fontFamily: "var(--font-cinzel), Cinzel, serif", fontSize: "clamp(14px, 1.8vw, 18px)", fontWeight: 600, color: WHITE, margin: "0 0 4px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {b.title}
        </h3>
        {b.subtitle && (
          <p style={{ fontFamily: "var(--font-inter), Inter, sans-serif", fontSize: 12, color: SILVER, margin: "0 0 4px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {b.subtitle}
          </p>
        )}
        {b.summary && (
          <p style={{ fontFamily: "var(--font-inter), Inter, sans-serif", fontSize: 11, color: MUTED, margin: 0, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
            {b.summary}
          </p>
        )}
      </div>
      <ExternalLink size={15} style={{ color: MUTED, flexShrink: 0 }} />
    </motion.a>
  );
}
